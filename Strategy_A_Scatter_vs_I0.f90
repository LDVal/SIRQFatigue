...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!Output
!File: Generates a table where the first column represents the initial fraction of infected individuals (I0), and the second column represents the corresponding fraction of recovered people R at the final stage. Each row corresponds to an individual realization.
...
Program MainProgram
  ...
  I0min  =0.001d0  ! Minimum proportion of infected population at t=0
  dI0    =0.002d0  ! Step size for varying the proportion of infected population
  I0n    =300      ! Number of iterations for the proportion of infected population.
...
subroutine SIR
  ...
  do while(Ninf>0.or.NisoI>0)
     ...
     enddo        
     !STEP 1: DETECTION AND QUARANTINE
     ...
     !STEP 2: CONTACT TRACING AND QUARANTINE
     NisoSnew           =0
     NisoIindirecnew    =0
     ListIndIsolSusNew  =0
     ...
     do i=1,NisoInew
        ...
        do j=1,kkp(nod)
           ...
           if(State(vec)==0)then
              !Susceptible individuals are placed under quarantine before 
              !being potentially infected by neighbors.
              NisoSnew      =NisoSnew+1
              State(vec)    =4
              ClockIso(vec) =1  !Note: ClockIso represents the time an 
                                !individual has been in isolation, and therefore 
                                !protected from being infected by other individuals.
                                !We set ClockIso = 1 to indicate immediate isolation upon potential exposure
     ...
     !STEP 3: INFECTION
     !Those infected individuals who have not been isolated, will now 
     !transmit the disease to their susceptible neighbors.
     ...
     do i=1,Ninf
        nod =ListIndInf(i)
        if(State(nod)==3)cycle  !infected people who have been placed under 
                                !quarantine in the previous step cannot transmit the disease further
        do j=1,kkp(nod)
           vec =edgep(nodep(nod)+j-1)
           if(State(vec)==0)then
              call rand
